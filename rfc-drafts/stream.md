...
At the moment, we do not see many `Unpin!` streams in practice (with [this exception in the futures-intrusive crate](https://github.com/Matthias247/futures-intrusive/blob/master/src/channel/mpmc.rs#L565-L625). Where they will become important is when we introduce async generators, as discussed in [future-possibilities].
